...
export default function NotesTable({ notes, onDelete, onEdit, editingNoteId }) {
  ...
  return (
    <div className="bg-black/40 backdrop-blur-sm border border-purple-500/20 rounded-2xl sm:rounded-3xl p-3 sm:p-4 md:p-6 lg:p-8">
      <h3 className="text-base sm:text-lg md:text-xl font-semibold text-white mb-3 sm:mb-4">Existing Notes</h3>
      
      {/* Mobile Card View - Shows on small screens */}
      <div className="sm:hidden space-y-3">
        {notes.length === 0 ? (
          <div className="text-center text-gray-500 py-4 text-sm">No notes found.</div>
        ) : (
          notes.map((note) => (
            <div
              key={note._id}
      ...
      <div className="hidden sm:block overflow-x-auto">
        <table className="w-full text-left text-gray-300 text-sm md:text-base">
          ...
          <tbody className="divide-y divide-purple-500/10">
            {notes.length === 0 ? (
              <tr>
                <td colSpan="6" className="py-4 text-center text-gray-500 text-sm">No notes found.</td>
              </tr>
            ) : (
              notes.map((note) => (
                <tr
